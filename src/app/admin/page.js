        ...
        <div className="bg-gray-950/40 p-1.5 rounded-3xl border border-gray-900/80 grid grid-cols-3 gap-1 sm:gap-2">
          {[
            ...
            <button
              ...
              className={`w-full h-14 rounded-2xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 touch-manipulation active:scale-95 ${abaAtiva === tab.id
                ...
                <tbody className="divide-y divide-gray-900/50">
                  {parceirosFiltrados.map((loja) => (
                    <tr key={loja.id} className="hover:bg-amber-500/5 transition-colors group">
                      ...
                      <td className="p-4">
                        <span className={`px-2 py-1 rounded-lg text-[9px] font-black uppercase border tracking-widest ${loja.status === "ativo" ? "bg-emerald-950 text-emerald-400 border border-emerald-800" :
              ...
              {parceirosFiltrados.map((loja) => (
                <div key={loja.id} className="bg-gray-950/40 border border-gray-900 rounded-[2rem] p-6 space-y-4 text-center">
                  <div className="flex justify-between items-start">
                    ...
                    <span className={`px-2 py-1 rounded-lg text-[8px] font-black uppercase border tracking-widest ${loja.status === "ativo" ? "bg-emerald-950 text-emerald-400 border border-emerald-800" : "bg-rose-950 text-rose-400 border border-rose-800"
                      }`}>
                      {loja.status}
                    </span>
                  </div>
                  ...
                  .map((c) => (
                    <div key={c.id} className="bg-gray-950/60 p-5 rounded-[2rem] border border-gray-900/80 flex flex-col lg:flex-row justify-between items-center gap-4 transition-all hover:border-amber-500/20">
                      ...
                      <div className="flex items-center gap-4 w-full lg:w-auto justify-between lg:justify-end">
                        <span className={`px-2 py-0.5 rounded-md text-[8px] font-black uppercase tracking-widest border ${c.status === "pendente" ? "bg-amber-950 text-amber-500 border border-amber-900/50" :
